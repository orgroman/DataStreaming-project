...
class RrctDetector(AnomalyDetector):
    ...
    def handleRecord(self, inputData):
        ...
        for tree in self._forest:
            ...
            tree_count += 1
            self._point_idx = len(tree.leaves)
            if len(tree.leaves) >= self._tree_size:
                # Insert the new point into the tree
                self._point_idx = int(random.random() * self._tree_size)
                tree.forget_point(index=self._point_idx)
            tree.insert_point(self._shingle_points, index=self._point_idx)
        ...
        #
        result_num = 1 - 1 / (1 + np.log(1 + median_codisp))
